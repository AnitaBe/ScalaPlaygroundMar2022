...
import com.github.AnitaBe.GutenberUtil.getTitle
import com.github.AnitaBe.MyUtil.getAuthor
...
object Day18ReadMultipleWorks extends App {
  ...
  def getBooksFromFiles(files:Array[String]):Array[Book] = {
    val bookArray = for (file <- files) yield {
      ...
      val author = GutenberUtil.getAuthor(lines)
      val title = GutenberUtil.getTitle(lines)
